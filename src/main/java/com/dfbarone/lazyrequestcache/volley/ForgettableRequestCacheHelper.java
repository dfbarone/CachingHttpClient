...
import com.dfbarone.lazyrequestcache.volley.request.ForgettableRequestCacheInterceptor;
import com.dfbarone.lazyrequestcache.volley.request.HeaderInterceptorJsonObjectRequest;
...
public class ForgettableRequestCacheHelper implements CacheHeaderInterceptor {
    ...
    // Get method
    public void requestString(final int method,
                            final String url,
                            final Request.Priority priority,
                            final VolleyCallback callback) {

        requestString(method, url, null, priority, 0, callback, this);
    }

    protected void requestString(final int method,
                                    final String url,
        ...
        final HeaderInterceptorStringRequest jsonRequest = new HeaderInterceptorStringRequest(method, url,
                ...
                new Response.ErrorListener() {
                    ...
                    public void onErrorResponse(VolleyError error) {
                        final String payload = tryLoadCachedRequest(url);
                        if (payload != null) {
    ...
    public <T> T tryLoadCachedRequest(String url, Class<T> clazz) {
        // Try to load the last request for this stream from http cache
        Cache.Entry e = tryLoadCacheEntry(url);
    ...
    public String tryLoadCachedRequest(String url) {
        // Try to load the last request for this stream from http cache
        Cache.Entry e = tryLoadCacheEntry(url);
        if (e != null && e.data != null) {
            String payload = ForgettableRequestCacheHelper.cacheEntryToString(e);
    ...
    public Cache.Entry tryLoadCacheEntry(String url) {
        // Try to load the last request for this stream from http cache
        Cache.Entry e = mRequestQueue.getCache().get(url);
        if (e != null && e.data != null) {
            return e;
